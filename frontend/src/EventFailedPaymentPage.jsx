...
const EventFailedPaymentPage = () => {
    ...
    try {
      ...
      const updated = await Promise.all(filtered.map(async (reg) => {
        if (reg.CheckoutReference) {
          try {
            const sumupRes = await apiFetch(`${apiUrl}/registrations/${reg.id}/sumup`, { credentials: 'include' });
            if (sumupRes.ok) {
  ...
  const handleMarkChased = async (registrationId) => {
    await apiFetch(`${apiUrl}/events/${eventId}/registrations/${registrationId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: "{\"FailedPaymentChased\": true}"
    })
      .then(res => res.json())
          ...
          <TableBody>
            {registrations.map((reg) => (
              <TableRow key={reg.id}>
                ...
                <TableCell>
                  {!reg.FailedPaymentChased && (<Button sx={{ mr: 1 }} onClick={() => handleMarkChased(reg.CustomerId)}>Mark as Chased</Button>)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
